...
class Boy:
    ...
    def update(self):
        ...
        # 물리 좌표계로 바꿔야 함.
        #self.x = clamp(0, self.x, server.background.w)
        #self.y = clamp(server.background.ch / 2, self.y, server.background.h - server.background.ch/2)

    def handle_event(self, event):
        self.state_machine.handle_event(('INPUT', event))

    def draw(self):
        #x,y는 배경 월드 좌표계를 , sx,sy 즉 화면 좌표로 변환
    ...
    def get_bb(self):
        return self.cx - 20, self.cy - 50, self.cx + 20, self.cy + 50
